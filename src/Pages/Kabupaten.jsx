...
const Kabupaten = () => {
  const { id, id2 } = useParams(); // Menggunakan satu useParams() untuk kedua parameter
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const responsePemilu = await fetch(
          `https://sirekap-obj-data.kpu.go.id/pemilu/hhcw/ppwp/${id}/${id2}.json`
        );
    ...
    fetchRouteData1();
    fetchRouteData2();
  }, [id, id2]); // Membuat useEffect bergantung pada kedua parameter
